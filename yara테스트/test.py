...
def filter_malicious_strings(strings, safe_strings):
    ...
    for s in strings:
        norm = normalize(s)
        if norm not in safe_set and len(norm) >= 6:
...
def save_safe_strings(hwp_paths, output_path="safe_strings.txt"):
    safe_set = collect_safe_strings(hwp_paths)
    with open(output_path, "w", encoding="utf-8") as f:
        for s in sorted(safe_set):
            f.write(s + "\n")
    print(f"[✓] 정상 문자열 {len(safe_set)}개 저장됨: {output_path}")
...
def analyze_malicious_hwp(hwp_path, output_dir="output", safe_strings_path="safe_strings.txt"):
    ...
    safe_strings = set(normalize(s) for s in load_safe_strings(safe_strings_path))
    streams = extract_streams_ole(hwp_path)
    rule_count = 0
